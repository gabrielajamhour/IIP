...
public class Calendar {
    ...
    /** Devuelve la representación como String de todos los eventos del calendario
     *  cuyo instante de inicio sea el TimeInstant t dado. Si no hay eventos en
     *  dicha situación, devuelve la cadena vacía.
     ...
     *  @return String.
     */
    public String eventsStartTimeAt(TimeInstant t) {
        String res = "";
        
        for (int i = 0; i < program.length; i++){  
            if(program[i] != null){
                TimeInstant instI = program[i].getStartTime().getInstant();
                if(instI.equals(t)){
                    res += program[i].toString();
                }
            }
